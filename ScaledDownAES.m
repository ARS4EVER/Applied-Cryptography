...
% bit plaintext for the scaled-down AES encryption.
% KeyHex = a length four hex string (four nibbles) representing the system key
% Output
% CtextHex = a length four hex string representing the resulting ciphertext.
%First we construct the round keys:
RoundKeysHexStr = ScaledDownAESRoundKeys(KeyHex);
%From this length 12 hex string, we extract the three keys as 2 by 2 nibble arrays:
