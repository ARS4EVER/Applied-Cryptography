...
% for i=1:length(STR)
% shift = mod(i,keylength);
% if shift == 0, shift = keylength; end %corresponding to the last character of keystring
% strOut(i) = ShiftCrypt(LCSTR(i),-Vec(ishift));
% end
% strOut = Int2LCText(UCTtext2Int(strOut));
for i=1:length(STR)
    ishift = mod(i,keylength);
    if ishift == 0, ishift = keylength; end %corresponding to the last character of keystring
    strOut(i) = ShiftDeCrypt(STR(i),Vec(ishift));
end